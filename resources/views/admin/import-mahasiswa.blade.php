...
@section('content')
  ...
  <!-- Header -->
  <div class="mb-6">
    <h2 class="font-display text-lg font-semibold">Import Akun Mahasiswa</h2>
    <p class="text-sm text-ink/50 mt-0.5">Tambahkan akun mahasiswa secara massal menggunakan file Excel</p>
  </div>

  <!-- Card: Langkah-langkah -->
  <div class="border border-line rounded-xl p-5 mb-6 bg-paper/40">
    <h3 class="text-sm font-semibold mb-3">Cara Import Akun Mahasiswa</h3>
    <ol class="space-y-2 text-sm text-ink/70">
      <li class="flex gap-2">
        <span class="flex-shrink-0 w-5 h-5 rounded-full bg-teal/10 text-teal text-xs font-mono flex items-center justify-center">1</span>
        Unduh template Excel terlebih dahulu dengan menekan tombol di bawah.
      </li>
      <li class="flex gap-2">
        <span class="flex-shrink-0 w-5 h-5 rounded-full bg-teal/10 text-teal text-xs font-mono flex items-center justify-center">2</span>
        Isi data dosen sesuai format kolom pada template (jangan mengubah nama kolom).
      </li>
      <li class="flex gap-2">
        <span class="flex-shrink-0 w-5 h-5 rounded-full bg-teal/10 text-teal text-xs font-mono flex items-center justify-center">3</span>
  ...
  <div class="border border-line rounded-xl p-5 mb-6 flex items-center justify-between">
    <div class="flex items-center gap-3">
      <div class="w-10 h-10 rounded-lg bg-teal/10 flex items-center justify-center">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="text-teal">
      ...
      <div>
        <p class="text-sm font-medium">Template_Akun_Mahasiswa.xlsx</p>
        <p class="text-xs text-ink/50">Format kolom: NPM, Nama, Email, Program Studi</p>
      </div>
    </div>
  ...
  <div class="border border-line rounded-xl p-5">
    ...
    <form onsubmit="event.preventDefault(); alert('File akan diproses (dummy, belum terhubung ke backend).');">
      <div
        id="dropZone"
      ...
      >
        <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="mx-auto mb-3 text-ink/30">
          <path d="M12 16V4"/>
          <path d="M7 9l5-5 5 5"/>
          <path d="M4 20h16"/>
        </svg>
