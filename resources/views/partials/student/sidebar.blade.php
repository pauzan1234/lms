...
<nav class="flex-1 overflow-y-auto px-4 py-5 space-y-6">
    ...
    <div class="space-y-1">
        <a href="{{ route('student.dashboard') }}"
            class="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-white/10 text-paper text-sm font-medium">
        ...
        <a href="{{ route('student.matakuliah.cari') }}"
        ...
        <a href="{{ route('student.akademik.quiz.index') }}"
            class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-paper/70 hover:bg-white/5 hover:text-paper text-sm font-medium transition-colors">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                stroke-width="1.8">
        ...
        <a href="{{ route('student.akademik.nilai.index') }}"
            class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-paper/70 hover:bg-white/5 hover:text-paper text-sm font-medium transition-colors">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                stroke-width="1.8">
                <path d="M3 3v18h18" />
    ...
    <div>
        <button @click="courseOpen = !courseOpen"
            class="w-full flex items-center gap-2 px-3 mb-2 text-[11px] font-mono uppercase tracking-wider text-paper/40">
            <span>Sedang Dibuka</span>
            <svg :class="courseOpen ? 'rotate-90' : ''" class="ml-auto transition-transform" width="12"
                height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="1.6">
                <path d="M4 2l4 4-4 4" />
            </svg>
        </button>
        ...
        <div x-show="courseOpen" x-cloak class="rounded-xl bg-white/5 border border-white/10 p-3">
            ...
            <p class="text-[11px] text-paper/40 font-mono mt-0.5 mb-3">
                {{ $mahasiswa->study_program ?? '-' }} · Semester {{ $mahasiswa->semester ?? '-' }} ·
